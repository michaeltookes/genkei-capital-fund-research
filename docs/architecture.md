...
**Resolution:** Added two narrow capabilities to `EndpointSchema`: `backfill_only=True` (freshness check samples the latest blob of any age and shape-checks it, but skips `NO_RECENT_SAMPLES`; a never-backfilled endpoint with zero blobs stays silent), and ordered `any_of_keys` with an optional value-shape guard (the first present interchangeable key in normalizer precedence must resolve to the normalizer-consumable shape, emitting `MISSING_ANY_OF_KEYS` when none do). The stablecoin spec now uses `required_keys=("symbol","name","pegType")` + `any_of_keys=("chainCirculating","chainBalances")` + `any_of_value_type="object"` + `backfill_only=True`. `watchlist drift` is clean; the either/or canary still fires if both keys vanish, both values become unparseable, or the selected preferred value becomes unparseable while a fallback key is also present.
