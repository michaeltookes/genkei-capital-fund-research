...
| **CI workflows** | `tests.yml` (push to main + PRs) plus per-source daily crons on the self-hosted runner: `defillama-daily.yml`, `coingecko-daily.yml`, `fred-daily.yml`, `sec-daily.yml`, `yahoo-daily.yml`, `cftc-weekly.yml`, `ishares-daily.yml`, `sui-staking-daily.yml`, `sui-unlocks-daily.yml`, `onchain-staking-daily.yml`, `etherscan-whales-daily.yml`, `coinbase-daily.yml`. Synthesis + signal emission: `watchlist-score-daily.yml` (B-065/B-097, daily 13:00 UTC — persists the composite scoring rubric then emits its band-crossing signals). Report generation (commits back to the repo): `signal-digest-weekly.yml` (D-024, weekly Mon 16:00 UTC). Reliability monitors: `workflow-failure-alert.yml` (B-071), `ingest-staleness-check.yml` (B-071), and `ingest-heartbeat.yml` (B-119). | R-019, R-020 |
...
### D-025 — Macro regime surfaces as per-stack overlay context, not a correlator co-signal
**Date:** 2026-06-21 · **In:** B-096 follow-up, `src/genkei/experiments/signal_macro_overlay.py`, `src/genkei/reports/signal_digest.py`
**Decision:** The macro-regime emitter (B-096) writes market-wide events on a sentinel `asset="MACRO"` / `horizon="macro:cross-sleeve:primary"`. Because `detect_stacks` groups events by `asset` and filters each rule to a single `horizon`, macro can never co-occur with per-asset signals in a stack — and we deliberately did **not** change that. Instead macro is surfaced as **overlay context**: `compute_stack_macro_contexts` reads the regime in effect at each stack's `window_end` and classifies it `corroborates` / `contradicts` / `neutral` vs the stack's direction; the weekly digest renders this as a `macro` column plus a current-regime header. Presentation layer only — the stack score is untouched (the same call B-100's `vs_bench` made). Regime→direction reuses the emitter's `direction_for_regime`, so overlay and emitted events can't disagree.
**Why:** "Macro is the spine; equities and crypto are downstream of macro" (CLAUDE.md) — the regime *modulates how to read* a per-asset stack (a bearish exit during `risk_off` is corroborated; during `risk_on` it's fighting the tape), it isn't itself a per-ticker signal. Modeling it as a co-source would have required diluting the `min_distinct_sources ≥ 2` gate (macro would corroborate every aligned asset at once) and inventing cross-horizon grouping — high blast radius for a semantically wrong framing.
**Alternative considered:** (a) Macro as a cross-asset co-source contributing to stack scores — rejected: changes core asset-grouping semantics and dilutes the multi-source gate. (b) Standalone "Macro regime" digest section with no per-stack tie — rejected as strictly less useful than per-stack alignment (kept the current-regime header as a bonus). (c) A plain `signal_rules.yml` rule — impossible given asset-grouping + single-horizon filtering.
**What would change our mind:** If a genuine second macro-horizon source lands (so `MACRO` could stack on its own) or the correlator gains cross-horizon overlay rules, macro could additionally participate in scoring; the overlay column stays regardless as the human-readable read.
...
### G-044 — editing an already-applied migration's `CREATE OR REPLACE VIEW` doesn't change the deployed view
**Hit:** surfaced 2026-06-21 building the B-096 macro-regime emitter; `load_regimes` and `genkei macro-regime` both errored `column "horizon" does not exist`.
**Symptom:** The 20260522 macro-regime view migration's body was later edited to add a `'macro:cross-sleeve:primary'::text AS horizon` column (and `macro_regime.load_regimes` + the CLI were updated to read it), but Alembic had **already** marked that revision applied — so the edited `CREATE OR REPLACE VIEW` never re-ran, and the deployed view stayed horizon-less. The mismatch was silent until something actually called the loader (the existing CLI was already broken; nobody had run it against the homelab). Worse, `CREATE OR REPLACE VIEW` couldn't have fixed it even if re-run: `horizon` sits mid-column-list (after `regime`), and Postgres only lets `OR REPLACE` *append* columns, not insert them — so it needs a `DROP VIEW` + `CREATE VIEW`.
**Resolution:** New forward migration (`20260621_fix_macro_regime_view_horizon`) that `DROP VIEW IF EXISTS` + recreates the view with the intended (horizon-bearing) definition. The emitter then reads `RegimeResult.horizon` cleanly.
**Avoid next time:** A migration file is a *historical record of one DDL event*, not a live spec — editing its body after it's applied changes nothing in any environment that already ran it (and silently diverges new clones from existing DBs). Schema changes always need a *new* forward migration. And remember `CREATE OR REPLACE VIEW` is append-only on columns; mid-list additions require drop+recreate.
