...
| `genkei.ingest.fred` | FRED collector. Reads `macro_series:` from `config/watchlists.yml`, hits `/series`, then `/series/observations` with a bounded realtime window. Daily mode starts at the latest stored `fred.observations.realtime_start` per series and drops rows clipped to that inclusive boundary unless the exact observation-date/vintage key already exists and needs a `realtime_end` update. `--backfill` requests full history from FRED's earliest realtime date. Lands two raw blobs per series (`series_<id>`, `observations_<id>`). Redacts the API key from the URL stored in `meta.raw_blobs`. | R-027 |
