...
**Resolution:** Added two narrow capabilities to `EndpointSchema`: `backfill_only=True` (freshness check samples the latest blob of any age and shape-checks it, but skips `NO_RECENT_SAMPLES`; a never-backfilled endpoint with zero blobs stays silent), and `any_of_keys` (at-least-one-present, emitting `MISSING_ANY_OF_KEYS` when none resolve). The stablecoin spec now uses `required_keys=("symbol","name","pegType")` + `any_of_keys=("chainCirculating","chainBalances")` + `backfill_only=True`. `watchlist drift` is clean; the either/or canary still fires if both keys ever vanish.
