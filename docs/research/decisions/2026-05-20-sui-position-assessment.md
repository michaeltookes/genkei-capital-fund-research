...
2. SUI token unlock schedule — **partially resolved by B-089** (2026-06-07): Phase 1 investigation surveyed 8 free sources and found only one viable path — CryptoRank's public Next.js SSR payload — which exposes the batch schedule for ONE of SUI's 8 allocation categories (Community Reserves, 10.648% of supply). The other 7 categories, including the **load-bearing Series A (7.142%), Series B (6.956%), and Early Contributors (6.134%) VC tranches that drive the actual bear thesis**, are paywalled across all surveyed free sources. New `onchain.sui_unlocks` table lands 85 monthly batches for Community Reserves (TGE 2023-05-03 through 2030-05-01). Headline query `SELECT SUM(unlock_tokens) FROM onchain.sui_unlocks WHERE unlock_date BETWEEN CURRENT_DATE AND CURRENT_DATE + 90` returns ~12M SUI in the next 90 days (~0.12% of total supply) — but this is Community Reserves only, not the full picture. Caller-side analytics must treat the table as a **partial picture, not a complete unlock schedule**. See `docs/sources/sui-unlocks.md` for the per-source survey + "what would unblock full coverage" path (Sui Foundation publishing a structured release schedule, or the paid-data budget opening per CLAUDE.md). The original "is continued dilution a known headwind or a surprise risk" question remains partly unanswered for the VC categories that matter most — the next SUI research session should weight position-sizing decisions with that limitation in mind.
3. ~~Validator / on-chain staking flow for Sui (Move-VM equivalent of the LINK B-082 ingester)~~ → **resolved by B-088** (2026-06-07): the public Sui mainnet fullnode JSON-RPC (`https://fullnode.mainnet.sui.io`) exposes `suix_getLatestSuiSystemState` + `suix_getValidatorsApy` without auth or rate limits, removing the need for the Blockvision third-party path the B-088 spec originally assumed. New `onchain.sui_validators` table lands one row per `(epoch, validator_address)` per daily run. Headline aggregate at first run (epoch 1151): 7.25B SUI staked across 129 validators with +2.2M SUI net pending inflow (commitments exceeding withdrawals — the canonical "are stakers committing capital or unbonding" answer). `genkei query "SELECT SUM(stake_amount_mist)/1e9 AS total_staked_sui, (SUM(pending_stake_mist) - SUM(pending_withdraw_mist))/1e9 AS net_pending_flow_sui FROM onchain.sui_validators WHERE epoch >= (SELECT MAX(epoch) - 30 FROM onchain.sui_validators) GROUP BY epoch ORDER BY epoch DESC"` will surface the 30-day trajectory once daily snapshots accumulate. **Backfill is NOT supported in v1** (the public RPC has no historical-epoch method); forward-only from 2026-06-07 onward, so the SUI 6-month outcome window will have ~3-5 months of staking-flow history to triangulate against by the November 2026 `/reflect-decisions` cycle.
4. ~~`defillama.stablecoins` historical sparsity~~ → **resolved by B-085** (2026-05-21): the daily ``/stablecoins`` endpoint returns current-state only, so we'd accumulated only the days since the collector started running (~14 days, SUI session). The per-asset ``/stablecoin/{id}`` endpoint via ``--backfill --since YYYY-MM-DD --endpoint stablecoins`` carries the historical timeseries; one-shot backfill landed multi-year per-(asset, chain) supply rows. Future research sessions can now run ``SELECT SUM(supply_usd) FROM defillama.stablecoins WHERE chain='Sui' AND ts::date IN (latest, latest-30, latest-90, ...)`` and get a real-time series.
