...
2. ~~SUI token unlock schedule~~ — **fully resolved by B-145** (2026-09-18; was partially resolved by B-089 2026-06-07): the B-089 Phase 1 investigation found only CryptoRank's un-gated "Community Reserves" sub-bucket (10.648% of supply) with the load-bearing Series A / Series B / Early Contributors VC tranches paywalled everywhere (tracked as B-115). When CryptoRank went behind a Cloudflare bot wall (2026-09), the collector was ported to DeFiLlama's open datasets bucket, which publishes **all 8 allocation series** (summing to the full 10B supply, TGE 2023-05-03 through 2030) — closing B-115 in the same change. `onchain.sui_unlocks` is now the **complete** unlock picture (227 batch rows; the legacy CryptoRank taxonomy rows were removed at switchover). Headline finding at port time: the VC tranches are nearly exhausted — Series A fully vested 2025-11, Series B 2026-05, Early Contributors finishes 2026-11 — leaving only ~1.0B SUI (~10% of supply) to unlock over the following 12 months, mostly Community Reserve drip. The "aggressive VC vesting" bear-thesis mechanism is structurally fading. See `docs/sources/sui-unlocks.md` for the v2 source contract.
