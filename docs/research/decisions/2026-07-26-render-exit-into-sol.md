...
3. **No valuation cushion:** 372× price-to-fees on ex-spike usage; the 06-29 deeper-exit condition is effectively met.
4. **Relative tape is rolling over** (−12.7pp vs SOL over 30d) with volume fading.
5. **The destination is favored by the desk's own signals:** SOL leads BTC 30d, Solana stablecoin supply is the only major-chain +$1.3B/30d grower, and SOL is a logged core holding — the same tactical-spec→discounted-core shape as the VIRTUAL→ETH rotation logged one day earlier.
