...
**Dark for IBM** — it was only added to the watchlist this session, so `sec.form4` (insiders), `13F` (crowding), and GDELT news matching have no IBM history yet (all three are forward-only, watchlist-driven), and the correlator has no IBM signal events. This is a genuine gap, flagged rather than papered over: the usual "who's positioning" cross-check (insider buys on the dip? institutional capitulation?) is unavailable for this call. The one positioning read I do have is the tape itself — the pre-warning roll-over (−12% from the ATH before Jul 14) plus the record 67M-share down day says distribution was already underway and the miss confirmed it. Analyst positioning post-miss: 15 Buy / 7 Hold / 1 Sell (23 covering), but targets are being cut — JPMorgan $291→$250, HSBC downgraded to Reduce/$191 — i.e. the sell-side is bifurcating, not uniformly bullish.
