...
confidence: medium
status: pending
trigger_reassessment: "CYBL files a reverse split + Nasdaq/NYSE uplisting OR reports a QoQ decline in shares outstanding (dilution reversing) OR a war-driven pop holds above ~$0.01 for >2 weeks"
---
...
- The catalyst (reverse split + uplist, or sustained share-count *reduction*) is **observable and absent** — management has had since 2022 and instead re-raised authorized shares and kept diluting. Absent that filing, the base rate for "$0.0016 OTC defense stock multi-bags durably" is brutally low.
...
- **What would flip me to "hold the lottery ticket":** a reverse-split + Nasdaq/NYSE uplisting filing, OR a reported QoQ *decline* in shares outstanding, OR a war-driven pop that holds above ~$0.01 for >2 weeks (the frontmatter trigger). None are present today.

## Conclusion

**Recommendation: do NOT buy/add; harvest the tax loss (sell) as the base case.** Adding to a diluting sub-penny stock is throwing good money after bad — averaging down here funds the dilution. Between *hold* and *harvest*, the rational portfolio move is to **realize the capital loss for tax-year 2026**: the loss is a certain, usable asset (offset realized gains; up to $3,000 against ordinary income; carry the remainder forward), whereas the hold is a structurally-compromised lottery ticket whose one path to value (a recap/uplist) management has declined to take for years while diluting. The expected value of "certain tax asset now" exceeds "speculative re-rate fought by dilution."
...
**Top risks to this call (what would make harvesting the wrong move):** (1) a near-term reverse-split + uplisting that re-rates the equity before a 31-day re-entry window closes; (2) a major-conflict escalation driving a war-news pop that *holds*; (3) the financials being better than the OTC-Pink snapshot suggests (the unreconciled +$2.85M earnings figure).
