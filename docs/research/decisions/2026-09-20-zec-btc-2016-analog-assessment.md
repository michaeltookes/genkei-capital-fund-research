...
1. ZCSH's next SEC quarterly fund snapshots confirm post-launch AUM above $2B with split-adjusted share count still expanding; if the data is not normalized, use only consecutive post-split checkpoints so the Sept 30 3-for-1 split is not counted as net creations (split lowers per-share price ~$32→~$11 post-3:1 — retail-friendlier ticket).
