...
inside the correlator window. Onset refs keep the legacy `chain:date` shape,
while ongoing refs carry the episode start date rather than the latest
observation date, so reruns update the live state without counting as
independent TVL evidence. Event strength is `max(acute, sustained)` so a deep
slow bleed isn't diluted toward zero by the acute conditions it doesn't trip.
Payload carries `stress_type` (`acute` / `sustained` / `both`) and
