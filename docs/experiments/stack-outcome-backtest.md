...
Every forward return is computed from the stack's knowable date forward — never backward. Most emitters are knowable at their event timestamp; delayed sources are shifted to availability first (13F `crowding` uses `period_of_report + 45 days`, matching the emitter's maturity gate). The backtest does not filter or weight stacks based on what happens *after* that anchor date. The unit tests pin this explicitly: `compute_stack_returns` takes `prices_by_asset` and a stack and never starts the return window before a delayed component could have been known.
