...
- **Resolved:** 2026-05-09 (branch phase-0-cleanup)
- **Outcome:** Container image swapped from `postgres:16-alpine` to `timescale/timescaledb:latest-pg16` on the Beelink (`docker compose down/up`, external `genkeicapital_postgres_data` volume preserved — no data loss). `ALTER SYSTEM SET shared_preload_libraries = 'timescaledb'` applied + container restarted to load the timescale shared library (the inherited PG data dir didn't have this set since it was created by plain Alpine PG). Alembic migrations applied cleanly: `meta` schema + `meta.ingest_runs` from `7d9d845497ae`, then `CREATE EXTENSION IF NOT EXISTS timescaledb` from `69f3fe427252`. Verified end-to-end — `\dx timescaledb` shows version 2.26.4 installed in `public`. The on-homelab compose backup at `docker-compose.yml.bak-20260509-152146` allows rollback to plain Alpine PG if ever needed.
