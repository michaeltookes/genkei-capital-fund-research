...
# Fixed equity-market benchmark. SPY is the natural peer for every
# watchlist equity — the equity-side analog of BTC for crypto. Stored
# as the Yahoo ticker so it joins ``yahoo.candles`` directly. SPY is
# the equity-core sleeve baseline per B-102's benchmarks watchlist
# section.
PEER_TICKER = "SPY"
PEER_SYMBOL = "SPY"
...
def _equity_assets(watchlist: Watchlist) -> list[tuple[str, str]]:
    """Return ``[(ticker, sleeve)]`` for watchlist equities, excluding SPY.
    ...
    engine should fire rel-strength stacks on.
    """
    out: list[tuple[str, str]] = []
    for entry in watchlist.equities:
        ticker = entry.symbol.strip().upper()
        if not ticker or ticker == PEER_TICKER:
            continue
        out.append((ticker, entry.sleeve or "core"))
    return out
...
def emit_recent_crossings(
    *,
    ...
    Loads the full available yahoo.candles series for each (asset, SPY)
    ...
    with db.ingest_run(
        ...
        if not assets:
            LOGGER.warning(
                "no watchlist equities (other than %s); equity rel-strength "
                "emitter has nothing to scope to",
                PEER_TICKER,
            )
            return EmitResult(
        ...
        peer_series = _load_price_series(PEER_TICKER, until=until)
        if not peer_series:
            LOGGER.warning(
                "no %s price data; equity rel-strength emitter cannot compute "
        ...
        for ticker, sleeve in assets:
            ...
            crossings = _detect_crossings(daily, asset=ticker)
