...
def _fmt_macro_cell(ctx: Any | None) -> str:
    """Render a stack's macro overlay as ``<regime> <glyph>`` (or n/a)."""
    if ctx is None or getattr(ctx, "regime", None) is None:
        return "n/a"
    glyph = _MACRO_ALIGNMENT_GLYPH.get(ctx.alignment, "—")
    return f"{ctx.regime} {glyph}"
...
def render_weekly_digest(
    ...
    *,
    ...
    macro_contexts: list[Any] | None = None,
    ...
    column). ``macro_contexts`` is the parallel list from
    ``compute_stack_macro_contexts`` (or None to omit the macro overlay
    column); ``current_regime`` is the latest ``RegimeResult`` for the
    header (B-096 follow-up). ``health_rows`` are ``meta.ingest_runs``
    health dicts (each with ``source`` / ``endpoint`` / ``age_hours`` /
    ``health_status``).
    """
    ...
    if current_regime is not None:
        lines.append(
            f"**Macro regime:** `{current_regime.regime}` "
            f"(as of {current_regime.ts.isoformat()}) — the prevailing "
            "cross-sleeve overlay; each stack below is tagged ✓ corroborated / "
            "✗ contradicted / ~ neutral vs this spine (B-096)."
        )
        lines.append("")
    ...
    show_macro = macro_contexts is not None
    # index stacks → their context positionally (parallel lists; avoid
    # zip(strict=) — unavailable on the 3.9 local test venv)
    bench_by_id = {}
    if show_bench:
    ...
    if show_macro:
        for i, s in enumerate(stacks):
            if i < len(macro_contexts):
                macro_by_id[id(s)] = macro_contexts[i]
    for horizon in horizons:
        ...
        for s in group:
            ...
            if show_macro:
                row += f" {_fmt_macro_cell(macro_by_id.get(id(s)))} |"
    ...
    if show_macro:
        legend += (
            "`macro` is the regime in effect at the stack's window_end "
            "(✓ corroborates the stack direction, ✗ contradicts, ~ neutral) "
            "— macro is the spine, equities/crypto are downstream (B-096). "
        )
...
def build_weekly_digest(
    *,
    ...
    if macro_overlay:
        ...
        macro_contexts = compute_stack_macro_contexts(stacks)
        try:
            current_regime = latest_regime()
        except Exception:  # pragma: no cover - header context is best-effort
            current_regime = None
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-macro-overlay",
        action="store_true",
        help="Skip the macro-regime overlay column + current-regime header.",
    )
    parser.add_argument(
        "--stdout",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    markdown, until = build_weekly_digest(
        ...
        macro_overlay=not args.no_macro_overlay,
    )
    if args.stdout:
        print(markdown)
    else:
