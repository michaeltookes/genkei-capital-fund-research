...
- **Crypto core (long-term hold)** — BTC, ETH, SOL, LINK, JUP, ZEC. JUP (added 2026-06-17) is a deliberate DeFi-adoption bet — dominant Solana DEX aggregator + on-chain perps — tracked both as a token (price) and as a protocol (TVL/fees via the `protocols:` Solana section). ZEC (added 2026-07-06) is a privacy-narrative bet — a BTC-analog by monetary design (21M cap, PoW, halvings) around zk-SNARK shielded transactions; the "BTC : ZEC :: HTTP : HTTPS" thesis. Small speculative starting size, held indefinitely; price, news, and shielded-pool adoption (no DeFi/TVL protocol fees; shielded-pool signal tracked in `zcash.shielded_pools` via `genkei zcash-usage`).
...
- `docs/backlog.md` — open items across 8 phases.
