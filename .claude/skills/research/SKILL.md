...
3. Fill in the frontmatter (date, asset, sleeve, horizon, confidence, status, trigger_reassessment). Use `status: pending` only when exposure or the tracked event is already active. For unfilled orders or other conditional records, use `status: inactive`, add a non-empty `activation_condition`, and make that condition say what flips the record to `pending` and which actual date becomes the reflection baseline. All required keys are validated by `tests/test_research_decisions.py`. If this decision replaces an older logged decision, also add `supersedes: <old-slug>`; use `related` only for contextual links that do not replace the prior call.
