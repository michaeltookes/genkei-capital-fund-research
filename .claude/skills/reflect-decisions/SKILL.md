...
8. If this is a scenario-ladder record and the completed `grade_date` close is not yet available, skip - not yet eligible for reflection unless a dated trigger/terminal event resolved it earlier. For ordinary records, if `elapsed_days < horizon_days`, skip - not yet eligible for reflection. Note in the run summary.
