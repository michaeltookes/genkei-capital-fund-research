...
- **Snapshot:** see `manifest.json` — pins the `coinbase` ingest-run ids this
  run read.
...
days earlier, and compute the simple return `latest / prior - 1`. A single SQL
query with a `ROW_NUMBER()` window per product does the point selection; the
notebook renders the ranked result as a DataFrame.
...
ranked. As of the pinned capture (see `manifest.json`, coinbase runs
108/1126/1127), the trailing-30d ranking was **SOL −5.5% > BTC −16.1% > ETH
−19.8%** — so the "BTC holds up best" hypothesis was *disproven* this window:
SOL had the shallowest 30d drawdown, BTC sat in the middle. A useful reminder
that the lowest-beta-anchor read is a multi-quarter tendency, not a guarantee
in any single 30-day window (and SOL had already fallen furthest over the
longer horizon, so a shallower recent slice is partly mean-reversion).
