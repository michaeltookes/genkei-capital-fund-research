...
## Acceptance criteria
- [x] `docs/repo-layout.md` exists with these sections:
...
- [x] `CLAUDE.md`'s "Open architectural decisions" entry for B-013 updated to point at `docs/repo-layout.md` instead of being marked open.
- [x] B-013 entry in `docs/backlog.md` updated with a `**Resolved by:** docs/repo-layout.md (YYYY-MM-DD)` line at the bottom; status remains `open` (the *layout decision* is resolved but the *migration work* lands in Phase 1, so don't move B-013 to `docs/resolved.md` yet — instead, narrow its scope to "carry out the migration per docs/repo-layout.md" and lower priority if it's now blocking nothing).
- [x] Tests still pass: `python3 -m unittest discover -s tests`.
- [x] No existing files moved or renamed in this mission.
...
Decided `src/genkei/{common,ingest,normalize,cli,experiments,reports}/` with tests mirroring under `tests/`. Decision record at `docs/repo-layout.md`. CLAUDE.md and backlog B-013 updated; B-013's scope narrowed to the migration itself (lands with B-017+ in Phase 1).
