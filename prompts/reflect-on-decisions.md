...
9. For `reflection_type: scenario_ladder`, skip if `today < grade_date` unless an explicit trigger/terminal event already resolved it. For ordinary records, skip if `elapsed_days < horizon_days` per the mapping above.
