...
**Early resolution beats the horizon math.** For `reflection_type: scenario_ladder`, a terminal trigger dated in frontmatter or derived from completed capped evidence is graded immediately as the scenario outcome it realized, even if the same file also has `superseded_by`; terminal ladder evidence takes precedence over a forward-link-only supersession. Otherwise, a decision that was superseded (frontmatter `superseded_by` set) resolves *now*, with a forward-link to the successor decision rather than a benchmark-paired outcome — see `docs/research/README.md` → "Supersession and trigger-fire". For ordinary action records, a trigger fire before horizon (`trigger_fired_at` / `trigger_fired: true`) also resolves with a forward-link instead of benchmark pairing. Don't grade ordinary records on a benchmark they were never held to, and don't leave superseded or terminal scenario records `pending` (a superseded decision left pending re-queues every run — the exact bug the B-118 dry run caught on the 2026-05-20 SUI decision). Resolve it with a short note or scenario outcome and count it as batch work to summarize and commit, even if no horizon-eligible decisions remain.
...
2. Skip if `status: resolved` or `status: deferred` (terminal — already handled), or `status: inactive` (pre-execution / awaiting activation). Scenario-ladder evidence that is merely late should not use terminal `deferred`; keep it `status: pending` with `scenario_status: pending_missing_evidence`.
3. Skip the template file `_template.md` and `README.md`.
4. Read optional `reflection_type`. If it is `scenario_ladder`, treat the file as a non-action scenario record: require date-only `grade_date`, allow optional date-only `scenario_window_start`, do not require or backfill `action`, and do not grade it on action-aware alpha. Do not skip it solely because `grade_date` is still in the future; every pre-grade run must first inspect the ladder's terminal predicate with completed/capped evidence, even when the file has no pre-populated `trigger_fired_at`, and derive the first matching terminal date when one exists. Run that terminal-predicate inspection before any manual-exit or scenario-evidence follow-up branch below is allowed to skip the record. Queue it for full scenario-ladder grading only after that check finds a terminal event or after a completed `grade_date` close is available (usually `today > grade_date`). If a scenario-ladder file also has `action`, stop and report the frontmatter conflict for manual repair rather than guessing.
...
6. **Manual-exit P&L follow-up:** this branch cannot preempt Step 4's scenario terminal-predicate inspection when the file is a scenario ladder. If `pnl_status: pending_missing_exit_inputs` is set, inspect the file's `exited_at` date and outcome note. If returned collateral value, final debt/carry, and realized net P&L are still missing, add the file to a `pnl_follow_up` list, leave `status: pending`, do not force a spot-price horizon grade, and skip remaining scan steps for this file. If those inputs have been supplied, resolve it with the actual leveraged-loop outcome: compute loop equity return from starting net equity to ending net equity over the actual `date` -> `exited_at` holding period, pull BTC over that same window, compute BTC benchmark return, raw alpha, and action-aware decision alpha, flip `status: resolved`, remove `pnl_status: pending_missing_exit_inputs` and `pnl_followup_reason` (or replace them with resolved P&L metadata), add it to a `pnl_resolved` list for the batch summary/commit, and skip remaining scan steps for this file.
7. **Scenario evidence follow-up:** if `scenario_status: pending_missing_evidence` is set, first run the scenario-terminal part of the early-resolution check below so a terminal price failure or other terminal predicate cannot be masked by missing follow-up evidence. If no terminal event is found, re-check the named missing inputs. If they are still unavailable, add the file to a `scenario_follow_up` list, leave `status: pending`, do not append an outcome block, and skip remaining scan steps for this file. If the evidence has arrived, remove `scenario_status` / `scenario_followup_reason` (or replace them with resolved evidence metadata) and continue so the scenario can be graded.
8. **Early-resolution check:** first handle scenario-ladder terminal triggers. If `reflection_type: scenario_ladder` while the file is still `pending`, inspect the ladder's terminal predicate before any supersession or grade-date skip, even when no `trigger_fired_at` frontmatter has been pre-populated. This is an evidence check, not just a frontmatter check: pull the completed/capped data needed to discover whether the terminal predicate has already fired. Use `trigger_fired_at` when present; otherwise pull the completed OHLC/non-price evidence needed only to test the terminal predicate from the scenario start boundary through the earlier of `grade_date` or the latest completed observation, then derive the first terminal-event date from that capped evidence. Compare the terminal date with `grade_date`, not `date + horizon_days`; if it is on or before `grade_date`, queue the file immediately for scenario grading capped at that terminal date, even when `superseded_by` is also set. Record any successor as context in the outcome, but do not let supersession skip the ladder's terminal state. Do not substitute the reflection run's `today` for a bare `trigger_fired: true`; if neither frontmatter nor completed evidence can supply the terminal date, report the missing terminal date/evidence for manual repair. After that scenario-terminal check, if `superseded_by` is set and the file is still `pending`, resolve it now with a forward-link to the successor (see "Early resolution beats the horizon math" above), add it to the `early_resolved` list for the batch summary/commit, and skip remaining steps for this file. If `trigger_fired_at` / `trigger_fired: true` is present before the normal horizon for an ordinary action record, keep the forward-link-only resolution.
9. Compute `elapsed_days = (today - frontmatter.date).days`.
10. For `reflection_type: scenario_ladder`, skip only after the terminal-predicate inspection found no terminal event and the completed `grade_date` close is not yet available. For ordinary records, skip if `elapsed_days < horizon_days` per the mapping above.
11. Add the rest to the to-reflect queue.

If the queue is empty, `early_resolved` is empty, `pnl_resolved` is empty, `action_backfilled` is empty, `pnl_follow_up` is empty, and `scenario_follow_up` is empty, report "no decisions past their horizon" and stop. If `pnl_follow_up` or `scenario_follow_up` has entries, report the missing inputs in the batch summary so the nonterminal queue stays visible; if there are no file edits from `early_resolved`, `pnl_resolved`, `action_backfilled`, or newly tagged scenario follow-ups, stop without committing. If `early_resolved` or `action_backfilled` has entries, skip realized-data pulling and benchmark math for those files. If `pnl_resolved` has entries, keep the manual-exit branch's actual holding-period loop return and BTC benchmark math, but skip ordinary horizon-based outcome pairing for those files. Then continue to the summary/commit path so resolved files, P&L-resolved manual exits, action-only frontmatter backfills, and newly tagged scenario follow-ups are persisted.
...
For each queued ordinary action decision, pull the price series from the decision date through today. Always include `--limit 1000` on `genkei prices` calls that use `--since` / `--until`; the CLI default is 30 rows, which is too short for the 180-day and 365-day reflection windows and can drop the decision-date endpoint. Scenario-ladder records use the bounded OHLC window defined in their own subsection below.
...
If frontmatter includes `reflection_type: scenario_ladder`, skip the action-alpha path. First establish the scenario market-evidence start boundary: use `scenario_window_start` when present; otherwise use `reflection_start.date` when present; otherwise use the decision `date`. For scenarios authored intraday, do not include a full decision-date daily candle unless timestamped/intraday evidence can exclude pre-decision trades. If only daily OHLC is available and the scenario's first thresholds or drawdown math could be satisfied by pre-decision same-day trades, add/use `scenario_window_start` set to the next full candle date and start there. Pull OHLC-capable asset candles from that start boundary through `grade_date`; for crypto records, use `coinbase.candles` directly or an OHLC-capable CLI/query surface, because close-only `genkei prices` output is insufficient for intraperiod threshold prints and peak-to-trough drawdowns. If a scenario-ladder OHLC query errors, returns empty, or cannot verify the required cap-date candle, keep `status: pending`, add/update `scenario_status: pending_missing_evidence` plus `scenario_followup_reason`, add the file to `scenario_followup_tagged` for the batch summary/commit, and retry on later runs rather than marking it `deferred`. If a dated terminal trigger resolves the scenario earlier, cap the series at that terminal date instead. `trigger_fired_at` is the preferred source; if only `trigger_fired: true` is present, derive the first terminal-event date from the capped evidence or report the missing terminal date for manual repair. If reflection runs after the cap date, do not include later candles in the high, low, drawdown, threshold, hold, or failure checks. Before resolving, verify that the cap-date candle was successfully collected or refreshed after the cap date fully ended and after any source-specific revision window (Coinbase daily candles run at 12:00 UTC and recent closes may revise within 24h). A date key, next calendar day, or noon-on-cap-date ingest alone is not evidence of completion; if a verified post-close refresh for the grade-date or terminal-date candle is unavailable, use the retryable missing-evidence path instead of terminal `deferred`. Then retrieve every load-bearing non-price input named by the ladder or `related` sources over the same capped window (for example ETF/fund snapshots or flows, second-issuer filing evidence, and shielded-pool supply). Do not invent missing price data or classify a tier whose required non-price evidence is unavailable. If load-bearing scenario evidence is expected but not yet published (for example a quarterly filing after `grade_date`), keep `status: pending`, add/update `scenario_status: pending_missing_evidence` plus `scenario_followup_reason`, add the file to `scenario_followup_tagged` for the batch summary/commit, and retry on later runs. Only mark a scenario `resolved` as `unresolved/mixed` when the evidence window has closed and absence of evidence is itself part of the realized outcome. The outcome block should name the realized tier(s), not `Action`, `Raw alpha`, or `Decision alpha`.
...
- For ordinary action records, if a pull errors or returns empty (source outage, delisted ticker, un-ingested name), mark the decision `status: deferred` with a note naming the gap — never fabricate. Scenario-ladder OHLC gaps use the retryable `scenario_status: pending_missing_evidence` path instead. The reflection cycle should still RUN — the failure mode of skipping reflection is worse than the failure mode of incomplete reflection.
...
- **Scenario-ladder facts:** compute the ladder facts directly using only OHLC observations from the scenario market-evidence start boundary through `grade_date` or the earlier dated terminal trigger: intraperiod high/low, grade-date or trigger-date close, peak-to-trough drawdown, named threshold hits, hold/failure conditions, and required non-price evidence from the named data sources. Record the highest threshold/path tier reached (base / extension / mania tail / ETH-flip tail / none) separately from the terminal state (held / base retrace / failure / unresolved) using the decision's own ladder language; do not collapse overlapping evidence into one winner. Do not compute action returns, action-aware decision alpha, or action-record confidence calibration.
- **Scenario probability read:** preserve the original confidence and stated probability ladder, but do not force one probability bucket onto a combined threshold-plus-terminal path unless the original ladder defines that joint bucket. If the ladder records threshold/path tiers and terminal states as separate dimensions, report their marginal probability reads separately (for example, "ETH-flip threshold tier (<5%) + terminal failure state (~5% marginal); no joint probability stated"). Do not label a single scenario realization as calibrated, underconfident, or overconfident by itself; reserve calibration judgments for an aggregate sample of comparable scenario forecasts.
...
- **Action confidence calibration:** use decision alpha, not raw alpha. If the decision said `confidence: high` and decision alpha is -5pp or worse, that's a calibration miss — note it. Same for `confidence: low` and a big positive surprise.
...
- **Probability read:** stated threshold/path probability and terminal-state probability separately when the ladder uses separate dimensions; use a joint probability only if the original ladder defined one. Note whether the realized evidence was base-case, tail, or surprising, with aggregate calibration deferred.
...
Then flip `status: pending` → `status: resolved` in the frontmatter, save, and commit. If no outcome block was written because scenario evidence is late, save and commit the `scenario_status: pending_missing_evidence` / `scenario_followup_reason` metadata instead so later runs keep retrying it. One commit per reflection batch is fine.
