...
5. **Manual-exit P&L follow-up:** if `pnl_status: pending_missing_exit_inputs` is set, inspect the file's `exited_at` date and outcome note. If returned collateral value, final debt/carry, and realized net P&L are still missing, add the file to a `pnl_follow_up` list, leave `status: pending`, and do not force a spot-price horizon grade. If those inputs have been supplied, resolve it with the actual leveraged-loop P&L and flip `status: resolved`.
