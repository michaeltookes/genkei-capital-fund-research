...
- **Scenario-ladder records:** compute the ladder facts directly using only observations through `grade_date`: intraperiod high/low, grade-date close, peak-to-trough drawdown, named threshold hits, hold/failure conditions, and required non-price evidence from the named data sources. Classify the scenario as base / extension / mania tail / failure / unresolved using the decision's own ladder language. Do not compute action-aware decision alpha.
