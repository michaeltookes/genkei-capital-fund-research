...
8. **Early-resolution check:** if `superseded_by` is set and the file is still `pending`, resolve it now with a forward-link to the successor (see "Early resolution beats the horizon math" above), add it to the `early_resolved` list for the batch summary/commit, and skip remaining steps for this file. If `trigger_fired_at` / `trigger_fired: true` is present before the normal horizon, keep the forward-link-only resolution for ordinary action records, but special-case `reflection_type: scenario_ladder`: require an actual terminal date from `trigger_fired_at` or derive the first terminal-event date from capped OHLC/non-price evidence. Compare that terminal date with `grade_date`, not `date + horizon_days`; if it is on or before `grade_date`, queue the file immediately for scenario grading capped at that terminal date. Do not substitute the reflection run's `today` for a bare `trigger_fired: true`; report the missing terminal date/evidence for manual repair.
